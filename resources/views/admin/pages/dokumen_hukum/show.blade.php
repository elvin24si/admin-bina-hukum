...
@section('content')
    <div class="container-fluid pt-4 px-4">
        ...
        <div class="bg-light rounded p-4 mt-4">
            <h6 class="mb-3">Riwayat Perubahan</h6>

            @if ($dokumen->riwayat->isEmpty())
            ...
            @else
                <table class="table table-bordered table-sm">
                    ...
                    <tbody>
                        @foreach ($dokumen->riwayat as $riwayat)
                            <tr>
                                <td>{{ $riwayat->tanggal }}</td>
                                <td>{{ $riwayat->versi }}</td>
                                <td>{{ $riwayat->uraian_perubahan }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="bg-light rounded p-4 mb-4">
            ...
            <form action="{{ route('dokumen_hukum.lampiran.upload', $dokumen->dokumen_id) }}" method="POST"
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                @if (str_starts_with($file->mime_type, 'image/'))
                                    <img src="{{ asset('storage/dokumen_hukum/' . $file->file_name) }}" width="60"
                                        class="img-thumbnail me-3">
                                @endif

                                <a href="{{ asset('storage/dokumen_hukum/' . $file->file_name) }}" target="_blank">
                                    {{ $file->caption }}
                                </a>
                            </div>
